...
Процедура ВыгрузитьФайлыИзКонфигурации()
	...
	ЧтениеJSON.ОткрытьФайл(ФайлНастроекПодключения);
	...
	Если НайтиФайлы(КаталогВыгрузки, "ConfigDumpInfo.xml", Ложь).Количество() <> 0 Тогда 
		Обновлять = "-update";
	Иначе
		Обновлять = "";
	КонецЕсли;		
		
	ПараметрыЗапуска.Добавить("/DumpConfigToFiles """ + КаталогВыгрузки + """ " + Обновлять);
